...
<script src="/fundcrm/Public/js/jquery-1.10.2.min.js"></script>
<link href="/fundcrm/Public/css/sandbox.css" rel="stylesheet" />
<script>
	$(document).ready(function() {
		$('#allotorder').click(function(){ 
	...
	function setsale(){
		var salename =  $(".salename:checked").val();
		var sales = $("#sales").val();
		$.ajax({
            url:__APP +"/Hefm/Order/setSale",
            type:'POST',
            data:"id=" + salename + "&sales=" + sales,
            dataType:'json',
            success:function (data){
            	if(data){
            		alert("分配成功");
            		window.location.href = __APP+"/Hefm/Order/orderList";
            	}else{
            		alert("分配失败");
            		window.location.href = __APP+"/Hefm/Order/orderList";
            	}
            ...
            <div class="widget-body">
                <div class="table-toolbar">
                    <div class="btn-group pull-right">
                        ...
                        <a class="btn btn-default dropdown-toggle" data-toggle="dropdown" href="javascript:void(0);"><i class="fa fa-angle-down"></i></a>
                           <?php if($checksetSale == true): ?><ul class="dropdown-menu dropdown-default">
                            <li>
                                <a href="javascript:void(0);" id="allotorder">分配订单</a>
                            </li>
                            <li class="divider"></li>
                            </ul><?php endif; ?>
                    </div>
                ...
                <table class="table table-striped table-hover table-bordered" id="editabledatatable">
                    <thead>
                        <tr role="row">
                        	<th>
                                <label><input  type="checkbox" id="checkall"><span class="text"></span></label>
                            </th>
                            <th>
                               	 姓名
                            </th>
                    ...
                    <tbody>
                    	<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
                        	<td><label><input  type="checkbox" value="<?php echo ($vo["id"]); ?>" class="orderid" name="orderid"><span class="text"></span></label></td>
                            ...
                            <td>
                                <a href="/fundcrm/index.php/Hefm/Order/orderinfo?id=<?php echo ($vo["id"]); ?>" class="btn btn-info btn-xs edit"><i class="fa fa-edit"></i> Edit</a>
...
<div class="theme-poptit">
	...
	<div class="widget-body bordered-bottom bordered-darkorange">
		<h5>销售人员列表</h5>
			<div class="row">
			    <?php if(is_array($sale)): foreach($sale as $key=>$value): ?><div class="col-lg-4 col-sm-4 col-xs-4">
				        <div class="checkbox">
				            <label>
				                <input type="radio" value="<?php echo ($value["id"]); ?>" name="salename"  class="salename">
				                <span class="text"><?php echo ($value["nickname"]); ?></span>
				            </label>
				        </div>
	...
	<button type="submit" class="btn btn-blue" onclick="setsale()">Submit</button>                                     
</div> 
</div> 
